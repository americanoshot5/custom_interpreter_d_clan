...
디버그 모드를 확인하는 방법은 두 가지이고, 목적이 다릅니다:

| | `demo4_debugging.py` | `factory_shell.py debug` (아래 참고) |
|---|---|---|
| 명령 | 스크립트에 미리 정해진 시나리오를 자동 재생 | `(debug)` 프롬프트에서 원하는 명령을 직접, 원하는 순서로 입력 |
| 용도 | **재현 가능한 데모** — 실행할 때마다 같은 결과가 나와서, watch로 `total` 값이 바뀌는 과정을 코드를 안 봐도 바로 확인 가능 | **직접 탐색/실험** — 다른 브레이크포인트나 다른 변수로 자유롭게 시도 |

즉 하나가 다른 하나를 대체하는 게 아니라, 데모용 고정 시나리오(`demo4_debugging.py`)와
실제 디버그 모드 CLI(`factory_shell.py debug`)로 서로 용도가 다릅니다.
