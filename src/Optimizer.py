...
# ── 고수준 Optimizer API ──────────────────────────────────────────────────────
#
# additional_optimizer_integration.py 에서 사용하는 공개 인터페이스:
#
...
class BindingTable:
    ...
    resolve_bindings() 가 반환하며, Executor 내부의 id-기반 BindingMap 과는
    별개로 사람이 읽기 좋은 이름-기반 인터페이스를 제공한다.
    """
    ...
    def lookup(self, name: str) -> BindingInfo:
        """name에 해당하는 BindingInfo(distance=...)를 반환한다."""
...
def resolve_bindings(program: Program) -> BindingTable:
    """
    프로그램 내 모든 변수 참조에 대해 이름 기반 스코프 거리 테이블을 반환한다.

    같은 이름이 여러 위치에서 참조된 경우, 가장 마지막으로 만난 참조의
    거리가 기록된다.
    """
    data: dict[str, int] = {}
...
def fold_constants(program: Program) -> Program:
    """
    상수 접기를 수행한 새 Program을 반환한다.

    반환된 Program은 optimization_stats() 로 접기 횟수를 조회할 수 있다.
    """
...
def optimization_stats(program: Program) -> dict[str, int]:
    """
    fold_constants() 로 생성된 Program의 최적화 통계를 반환한다.
...
def optimize(program: Program) -> Program:
    """
    가능한 모든 컴파일 타임 최적화를 적용한 새 Program을 반환한다.

    현재 적용 순서:
      1. 상수 접기 (ConstantFolder) — 리터럴 연산 사전 계산
      2. 정적 바인딩 분석 (StaticBinder) — 바인딩 정보는 Executor 에 전달 시 활용

    반환된 Program은 check() 와 execute() 를 정상적으로 통과한다.
    """
